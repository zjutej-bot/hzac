...
export default function PlayerGame() {
  ...
  const sorted = [...myDraftedPlayers].sort((a, b) => {
  ...
  return (
    <div className="min-h-screen bg-white">
      {showInterestPopup && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full mx-4 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">利息结算</h3>
            {interestAmount > 0 ? (
              <p className="text-gray-600 mb-2">获得了 <span className="text-red-600 font-bold text-xl">{interestAmount}</span> 元利息</p>
            ) : (
              <p className="text-gray-600 mb-2">余额不足5元，未获得利息</p>
            )}
      ...
      <div className="max-w-2xl mx-auto px-8 py-6">
        {message && <div className="mb-4 p-3 rounded text-sm bg-red-50 border border-red-200 text-red-700">{message}<button onClick={() => setMessage('')} className="ml-2 underline">关闭</button></div>}
        ...
        {(game.current_phase === 'draft' && isFinal) && (
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            {myDraftedPlayers.length === 0 ? <p className="text-gray-400 text-sm py-2">空大名单</p> : (
              <div className="space-y-2">{sorted.map(r => (<div key={r.id} className={`flex items-center p-3 rounded border ${r.status === 'final' ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50'}`}><span className="text-sm text-gray-900">{r.name}</span><span className="text-xs text-gray-500 ml-2">({r.cost}费)</span>{r.status === 'final' && <span className="text-xs text-red-600 font-medium ml-2">大名单</span>}</div>))}</div>
            )}
          </div>
        )}
