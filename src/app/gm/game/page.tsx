...
export default function GMGame() {
  ...
  const submit = async () => {
    ...
    for (const [uid, rank] of Object.entries(ranks)) {
      ...
      if (u) await supabase.from('users').update({ money: (u.money || 0) + fixedBonus, score: (u.score || 0) + bs }).eq('id', uid)
    }
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="border-b border-gray-200 bg-red-50"><div className="max-w-4xl mx-auto px-8 py-4 flex justify-between items-center"><div><h1 className="text-lg font-bold text-red-700">游戏局{game.game_number} (GM)</h1><p className="text-sm text-gray-500">第 {cr}/{maxRounds} 赛季 · {phase}</p></div><div className="flex items-center gap-2"><button onClick={endGame} className="px-4 py-1.5 bg-red-600 text-white rounded hover:bg-red-700 text-sm">终止游戏</button><button onClick={() => router.push('/gm/dashboard')} className="px-4 py-1.5 border border-gray-300 text-gray-600 rounded hover:bg-gray-50 text-sm">退出</button></div></div></div>
          ...
          <div className="col-span-2 space-y-4">
            ...
            {game.current_phase === 'match' && (
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm font-medium text-gray-700 mb-2">S{cr} 奖励明细</p>
