        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div>
            {game.current_phase === 'draft' && allDone() && (
              <button onClick={enterMatch} className="px-5 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium">进入比赛阶段</button>
            )}
            {game.current_phase === 'match' && (
              <button onClick={openRank} className="px-5 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium">提交比赛成绩</button>
            )}
          </div>
        </div>
